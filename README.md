...
- Ally: a general chat assistant (floating widget on every page, plus a full `/bot` page), backed by Google's Gemini API directly (`/api/chat`). Stateless by design — the client sends the recent conversation history with every request, and nothing is persisted server-side afterward, on this app's end or Google's
...
| Ally chat replies | `GEMINI_API_KEY` (without it, `/api/chat` returns a clear "not configured" error rather than crashing) |
...
- `/` — home: a welcome hub with quick links to Journey, Safety Tools, Contacts, and Ally
...
- `/bot` — full-page Ally (also available as a floating widget on every page)
- `/auth` — sign up / log in
