    ...
    <>
      ...
      <main style={{ flex: 1, padding: "1.5rem 1.5rem 3rem", maxWidth: 1100, margin: "0 auto", width: "100%", display: "flex", flexDirection: "column", gap: "2.5rem" }}>
        <section id="features" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "1.25rem" }}>
          ...
          <AboutCard icon={<Bot size={20} />} title="Ally" desc="Ask about an upcoming trip and get a safety-aware forecast for the best time and mode to travel." />
        </section>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", width: "100%", display: "flex", flexDirection: "column", gap: "2.5rem" }}>
          <section className="card" style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            <p style={{ fontSize: "0.9rem", color: "var(--foreground-muted)", lineHeight: 1.6 }}>
              <T>
                {`HerLane's routing and safety signals run on OpenStreetMap-based services (Photon, OSRM, Overpass) — no paid API keys or billing accounts required to use the core app. Where a feature depends on an optional service (like SMS delivery or Ally's natural-language replies), it's clearly labeled and degrades gracefully rather than pretending to work when it can't.`}
              </T>
            </p>
          </section>
